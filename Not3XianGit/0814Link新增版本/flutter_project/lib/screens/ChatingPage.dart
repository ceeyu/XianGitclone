...
class _ChatingPageState extends State<ChatingPage> {
  ...
  Future<String?> getLink() async {
    String? Link = await _storage.read(key: 'link');
    if (kDebugMode) {
      print('RoomLink: $Link');
    }
    return Link;
      ...
      try {
        ...
        if (response.statusCode >= 200 && response.statusCode < 300) {
          ...
          if (responseData.isNotEmpty &&
              responseData[0]['message'] != '他倆沒有聊天紀錄') {
    ...
    if (savedAccessToken != null) {
      try {
        ...
        if (response.statusCode >= 200 && response.statusCode < 300) {
          ...
          if (message.startsWith('Someone sent you a link :') &&
              responseData[0]['isLink'] == 'true') {
            isLinkMessage = true;
          } else {
            isLinkMessage = false;
          }
  ...
  Future<void> joinLinkRoom(String link) async {
    ...
    if (savedAccessToken != null) {
      try {
        ...
        if (kDebugMode) {
          print('Join Link Room Catch Error: $error');
        }
      }
    }
  }
    ...
    if (savedAccessToken != null) {
      try {
        ...
        if (kDebugMode) {
          ...
          print(//檢查用
              "AppId: $APP_ID, Room_UUID: $ROOM_UUID, Room_Token: $ROOM_TOKEN");
        }
        if (response.statusCode >= 200 && response.statusCode < 300) {
          final responseData = jsonDecode(response.body);
          setState(() {
              ...
              child: chatDataList.isEmpty
                  ...
                  : ListView.builder(
                      ...
                      itemBuilder: (context, index) {
                        ...
                        bool isSender = chat['sender_firstname'] ==
                            widget.selectedName; //not me
                        Color bubbleColor = isSender
                            ? const Color.fromARGB(255, 14, 159, 203)
                            : const Color.fromARGB(255, 14, 100, 44);
                        if (isLinkMessage || chat['isLink'] == 'true') {
                          return InkWell(
                            ...
                            child: BubbleSpecialThree(
                              text:
                                  'Someone sent you a link : ${chat['chat_data']}',
                              color: bubbleColor,
                              ...
                              tail: true,
                              isSender: chat['sender_firstname'] !=
                                  widget.selectedName,
                            ),
                          );
                        } else {
                          return BubbleSpecialThree(
                            ...
                            tail: true,
                            isSender:
                                chat['sender_firstname'] != widget.selectedName,
                          );
                        }
                      },
                    ),
